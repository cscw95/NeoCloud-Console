...
Optionally start the NOCP backend
([NeoCloud-Control-Plane](https://github.com/cscw95/NeoCloud-Control-Plane),
`./run.sh`, port 8000) — the consoles auto-detect it and switch to live data
(a "NOCP live" badge appears in the top bar). Without it, every screen falls
back to scenario mock data.

For the **full four-tier stack** — consoles (:8090) → NOCP (:8000) →
[NICo Emulator](https://github.com/cscw95/NICo-Emulator) (:9000) →
[AI Infra Emulator](https://github.com/cscw95/AI-Infra-Emulator) (:9100,
VR NVL72 physical twin) — follow the
[E2E runbook](https://github.com/cscw95/NeoCloud-Control-Plane/blob/main/docs/E2E_RUNBOOK.md)
(bring-up order, demo staging, `scripts/e2e_full_chain.py` 12-check proof).
The ops console's **Managed K8S** sidebar group renders the operator-portal
mockup inline (`ops/mk8s.{css,js}`, generated by `ops/build-mk8s-inline.py` from
`ops/managed-k8s.html` — never edit the generated files directly).

### Managed K8S — Mock/Live dual mode

The sidebar group header carries a **Mock/Live toggle**
(`localStorage["nc-mk8s-mode"]`, default mock). *Mock* keeps the original
self-contained demo dataset pixel-identical. *Live* drives every one of the
10 sub-views from the NOCP Managed K8s API (`/api/v1/k8s/*`) through a data
provider layer (`DS` in `managed-k8s.html` — MockDS wraps `NEOCLOUD_DATA`,
LiveDS polls REST with a 2.5 s snapshot cache). Switching to Live probes
`GET /api/v1/spec`; if NOCP (:8000) is down it toasts and reverts. The demo
reset button is mock-only.

Live mode covers both provisioning paths end-to-end against the emulator
chain (NOCP → NICo :9000 → AI Infra :9100):

- **Day-1** — a BMaaS order with `managed_k8s: true` passes the
  `k8s_installing` gate; the delivery card deep-links to the cluster detail.
- **Day-2** — the "cluster create" wizard picks a *delivered* allocation,
  an N/N-1 version and optional add-ons, POSTs `/api/v1/k8s/installs`, then
  live-polls the 8-stage NKD install stepper (cp-reserve → os-provision →
  net-attach → nkd-bootstrap → addons → acceptance → telemetry → active).

Operational scenarios that work live: kubeconfig issue/revoke (fake PKI
serial + TTL), rolling cluster upgrade (cordon→drain→upgrade→uncordon per
node), and GPU fault injection (`POST /api/v1/emu/faults`) surfacing in the
NVSentinel view as a quarantined worker with a hot-spare proposal and TTL
auto-recovery.
